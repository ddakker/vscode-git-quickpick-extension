...
function renderBranchRow(branch, isRemote, config, labels, branchHistory, expanded, commitFiles, branchHistoryHasMore, expandedKeys = [], fileViewMode = 'list') {
  ...
  if (isExpanded) {
    ...
    let commitHtml = renderCommitTable(
      branchHistory[branch.name], 'branchHistoryCommit', config, labels, commitFiles, expandedKeys,
...
function renderLists(state, labels) {
  ...
  if (exp.history) {
    let body = renderCommitTable(state.history, 'historyCommit', config, labels, cf, expandedKeys, 'history', fileViewMode);
