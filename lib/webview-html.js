...
<style>
  * { box-sizing: border-box; }
  ...
  .toolbar { order: 0; display: flex; gap: 2px; padding: 4px;
...
<body class="pos-${pos}">
  <div class="toolbar">
    ...
    <button data-cmd="gitReflow.execForcePush" title="${esc(labels.tbForcePush)}">⇪</button>
  ...
  <script nonce="${nonce}">
    ...
    lists.addEventListener('contextmenu', (e) => {
      const target = e.target.closest('[data-kind="commit"], [data-kind="branch"]');
      if (!target) return;
      e.preventDefault();
      const kind = target.dataset.kind;
      showMenu(e.clientX, e.clientY, MENU[kind] || [], target.dataset);
    });
      ...
      for (const it of items) {
        ...
        d.addEventListener('click', (ev) => {
          ...
          vscode.postMessage({ type: 'command', command: it.command,
            arg: { kind: data.kind, hash: data.hash, branch: data.branch, ctx: data.ctx, unfetched: data.unfetched === '1' } });
        });
        menu.appendChild(d);
      }
    ...
    lists.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowDown') { e.preventDefault(); idx = idx < 0 ? 0 : Math.min(rows.length - 1, idx + 1); rows[idx].focus(); }
        ...
        if (el.classList.contains('commit-row')) {
          ...
          showMenu(r.left + 12, r.bottom, MENU.commit, el.dataset);
        } else { el.click(); }
