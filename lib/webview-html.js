...
function renderBranchRow(branch, isRemote, config, labels, branchHistory, expanded, commitFiles, branchHistoryHasMore) {
  ...
  let sub = '';
  if (isExpanded) {
    let commitHtml = renderCommitTable(branchHistory[branch.name], 'branchHistoryCommit', config, labels, commitFiles);
...
function renderLists(state, labels) {
  ...
  if (exp.history) {
    let body = renderCommitTable(state.history, 'historyCommit', config, labels, cf);
  ...
  if (exp.localBranch) {
    const rows = (state.localBranches || []).map(b =>
      renderBranchRow(b, false, config, labels, state.branchHistory || {}, exp, cf, hasMore)).join('');
  ...
  if (exp.remoteBranch) {
    const rows = (state.remoteBranches || []).map(b =>
      renderBranchRow(b, true, config, labels, state.branchHistory || {}, exp, cf, hasMore)).join('');
    parts.push(`<div class="section-body">${rows || `<div class="empty">${esc(labels.noBranches)}</div>`}</div>`);
  }
  parts.push(`</div>`);
...
<style>
  * { box-sizing: border-box; }
  ...
  .chactions-merge .chaction:hover { color: var(--vscode-foreground);
                                     background: var(--vscode-toolbar-hoverBackground, var(--vscode-list-hoverBackground)); }
  .conflict-merge-hint { font-size: 0.8em; color: var(--vscode-charts-green, #89d185);
                         opacity: 0.85; margin-left: 6px; white-space: nowrap; flex-shrink: 0; }
