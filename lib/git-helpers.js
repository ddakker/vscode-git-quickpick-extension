...
function buildGitEnv() {
  ...
  // 한글 등 비ASCII 파일명이 \354\... 같은 8진수 이스케이프로 출력되지 않도록
  const idx = Number(env.GIT_CONFIG_COUNT || 0);
  env.GIT_CONFIG_COUNT = String(idx + 1);
  env[`GIT_CONFIG_KEY_${idx}`] = 'core.quotepath';
  env[`GIT_CONFIG_VALUE_${idx}`] = 'false';
  return env;
}
...
/**
 * git 실행 에러에서 사용자에게 보여줄 메시지를 추출.
 * husky/lint-staged 훅 실패 상세는 stdout 으로 나오는 경우가 많아
 * stderr 만 보면 이유가 비어버린다 → stderr + stdout 을 합쳐서 반환.
 */
function formatGitError(err) {
  if (!err) return '';
  const combined = ((err.stderr || '') + (err.stdout || '')).trim();
  return combined || (err.message ? err.message.trim() : String(err));
}

/**
 * git 에러 출력이 훅(pre-commit 등) 실패인지 판정.
 * husky/lint-staged 를 포함해 흔한 git 훅 이름이 출력에 있으면 훅 실패로 본다.
 */
function isHookError(output) {
  return /pre-commit|pre-push|commit-msg|prepare-commit-msg|post-commit|husky|lint-staged|\bhook\b/i
    .test(output || '');
}
